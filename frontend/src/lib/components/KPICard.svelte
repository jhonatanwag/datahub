...
<div class="kpi-card card" style={estiloCard}>
  ...
  {#if descricao}
    <p class="kpi-descricao" style={estiloLabel}>{descricao}</p>
  {/if}
</div>

<style>
.kpi-card { display: flex; flex-direction: column; gap: 6px; }
.kpi-topo { display: flex; flex-direction: column; gap: 6px; }
...
.kpi-descricao {
  ...
  border-top: 1px solid var(--border); text-transform: none; letter-spacing: normal;
}
